...
base_path = r"G:\Meu Drive\features_py\savee\AudioData\AudioData\**\*.wav*"
...
for i in range(1, 401):
    if i == 1:
        ica = FastICA(n_components = 2, algorithm= "parallel", whiten="unit-variance", fun='logcosh', fun_args={'alpha' : 1.0}, tol=1e-4, max_iter=500, w_init=None)   
        ...
        ica = FastICA(n_components = i, algorithm= "parallel", whiten="unit-variance", fun='logcosh', fun_args={'alpha' : 1.0}, tol=1e-4, max_iter=500, w_init=None)   
...
excel_filename = 'savee_pca_ica_kw2.xlsx'


with pd.ExcelWriter(excel_filename, engine='openpyxl') as writer:
    
    pd.DataFrame(feats_dfs).to_excel(writer, sheet_name='feats_stand', index=False)
    pd.DataFrame(feats_df).to_excel(writer, sheet_name='feats_df', index=False)
    pd.DataFrame(df).to_excel(writer, sheet_name='emotions', index=False)
    pd.DataFrame(norm_all).to_excel(writer, sheet_name='norm_all', index=False)
    pd.DataFrame(not_norm_all).to_excel(writer, sheet_name='not_norm_all', index=False)
    pd.DataFrame(feats_kwn).to_excel(writer, sheet_name='feats_kwn', index=False)
